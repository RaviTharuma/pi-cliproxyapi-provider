...
describe("Codex SSE fallback transport patch", () => {
	it("keeps stock pre-stream SSE fallback and records fallback after a started WebSocket failure", () => {
		...
		expect(patched).toMatch(/if \(websocketStarted\) \{\s*recordWebSocketSseFallback\(cacheSessionId\);\s*throw error;/);
